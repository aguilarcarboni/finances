...
class AssetsManager: ObservableObject {
    ...
    private func loadMockAssets() {
        ...
        // Add revenue-generating business asset
        let business = Asset(
            ...
            monthlyRevenueTarget: 400000, // 400K CRC monthly target
            revenueGenerationRate: 0.75, // 75% chance of hitting target
            variabilityFactor: 0.4 // Revenue can vary by ±40%
        )
        addAsset(business)
        
        // Add rental property with active loan
        let rentalProperty = Asset(
            name: "Rental Apartment",
            type: "Real Estate",
            category: .tangible,
            acquisitionDate: Calendar.current.date(byAdding: .year, value: -3, to: Date()) ?? Date(),
            acquisitionPrice: 45000000, // 45M colones
            currentMarketValue: 52000000, // Appreciated value
            customDepreciationRate: 0.04, // 4% annual appreciation
            expenseCategory: "Real Estate",
            loanAmount: 36000000, // 80% financing
            interestRate: 9.0,
            loanTermYears: 15,
            downPayment: 9000000, // 20% down
            monthlyRevenueTarget: 280000, // 280K CRC monthly rent
            revenueGenerationRate: 0.9, // 90% occupancy rate
            variabilityFactor: 0.1 // Rent is fairly stable
        )
        addAsset(rentalProperty)
        
        // Add an example of intellectual property (royalty generating)
        let patent = Asset(
            ...
            monthlyRevenueTarget: 75000, // 75K CRC monthly royalties
            revenueGenerationRate: 0.6, // 60% chance of generating revenue
            variabilityFactor: 0.8 // Royalties can be very variable
        )
        addAsset(patent)
        
        // Generate some historical revenue data for testing
        generateHistoricalRevenues()
    }
    ...
    // MARK: - Revenue Generation Simulation
    
    /// Simulates monthly revenue generation for all revenue-generating assets
    func simulateMonthlyRevenues(for date: Date = Date()) {
        for var asset in assets {
            if asset.isRevenueGenerating {
                asset.emitRevenueToExpensesAccount(for: date)
            }
        }
    }
    
    /// Generates historical revenue data for testing purposes
    func generateHistoricalRevenues() {
        let calendar = Calendar.current
        let currentDate = Date()
        
        for var asset in assets where asset.isRevenueGenerating {
            // Generate revenue for the last 12 months
            for monthOffset in 1...12 {
                if let monthDate = calendar.date(byAdding: .month, value: -monthOffset, to: currentDate) {
                    // Only generate for months after asset acquisition
                    if monthDate >= asset.acquisitionDate {
                        asset.emitRevenueToExpensesAccount(for: monthDate)
                    }
                }
            }
        }
    ...
    /// Returns all revenue-generating assets
    var revenueGeneratingAssets: [Asset] {
        return assets.filter { $0.isRevenueGenerating }
    }
    
    /// Returns total monthly revenue target across all assets
    var totalMonthlyRevenueTarget: Double {
        return revenueGeneratingAssets.reduce(0) { $0 + $1.monthlyRevenueTarget }
    }
    
    /// Returns actual total revenue generated by all assets
    var totalRevenueGenerated: Double {
        return revenueGeneratingAssets.reduce(0) { $0 + $1.totalRevenueGenerated }
    }
} 
